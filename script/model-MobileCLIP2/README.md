...
This guide covers the current Android model contract, reproducible ONNX/TFLite/ORT exports, and separate accuracy and timing experiments. Run commands from the repository root. Write new experiment outputs under ignored `build/` directories. The retained summaries describe historical artifacts and runtimes; raw measurement files are not distributed.
...
Exporter metadata stores input provenance as repository-relative paths. Inputs outside the checkout are identified by basename and SHA256; local absolute paths are not recorded.
...
Git retains [aggregate measurements, model hashes and protocols](results/measured-summary.json), not raw logs, per-image predictions, input manifests or feature arrays. Recomputing rankings and paired statistics requires a local rerun that generates the corresponding inputs and outputs under `build/`.
...
Run the checks against your current checkout and keep generated logs under `build/`. Offline regressions validate code and retained summary consistency; they do not recreate the historical accuracy or speed measurements.
...
The default accuracy configuration includes nine entries, including offline FP32-text controls. The four download ZIPs omit those reference files. `prepare_accuracy_models.py` generates the v1 references; the v2 controls also need `build/mobileclip2-reference/mobileclip2_s0_text.onnx`, generated by the ONNX source exporter above. TFLite entries require Python `ai-edge-litert==2.1.4`, separate from Android's LiteRT dependency.

Use the full Python environment and the following sequence. The v2 source export stages its deployment ONNX files under `build/`, preserving the downloaded App/ORT pairs while creating the missing FP32 text reference:


```bash
python -m pip install ai-edge-litert==2.1.4
python script/model-MobileCLIP2/export_mobileclip2_onnx.py --output-dir build/mobileclip2-reference/exported-onnx --reference-dir build/mobileclip2-reference
...
The ORT matrix requires the v1 exports, current v2 ONNX/ORT pair and historical CLIP assets. The [historical v1/v2 accuracy summary](results/README.md#historical-onnxtflite-accuracy) used an earlier runtime; the [ORT comparison](results/ort-comparison/README.md) records its own ORT 1.29.0 host/phone protocol. A rerun must report its actual versions and hashes; do not combine historical accuracy with new timing into a single claimed configuration.
...
The [measurement index](results/README.md) separates historical protocols and retained aggregates. Earlier App/optimization measurements used Android ONNX Runtime 1.23.2 / LiteRT 1.4.1, including an initial Java lazy-delegate threading issue corrected with an explicit native delegate. Those observations do not establish current-runtime performance. Only the compact measured JSON and paired English/Chinese summaries remain; raw diagnostic reports must be regenerated locally.
