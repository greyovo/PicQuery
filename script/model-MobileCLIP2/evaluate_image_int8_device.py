...
def render(result, host, args, output, language="en"):
    ...
    lines += ["", t("## Validation and scope", "## 验证与范围"), "",
              ...
              t("Raw .f32 feature arrays and large model/input files are generated locally and excluded from Git. Archived JSON retains measurements, per-image predictions, pairing statistics and hashes. A fresh checkout can audit these records; recomputing complete cosine rankings requires regenerating the phone feature files first.", "原始 .f32 特征数组与大型模型、输入文件在本地生成，不纳入 Git。归档 JSON 保留测量值、逐图预测、配对统计和哈希。新克隆可审计这些记录；重新计算完整余弦排序须先在手机重新生成特征文件。"), "",
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=Path("script/model-MobileCLIP2/results/image-int8/pixel8a-accuracy"))
    parser.add_argument("--host", type=Path, default=Path("script/model-MobileCLIP2/results/image-int8/accuracy.json"))
    parser.add_argument("--evaluation-manifest", type=Path, default=Path("script/model-MobileCLIP2/results/image-int8/accuracy-manifest.json"))
