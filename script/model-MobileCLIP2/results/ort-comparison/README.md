...
The [device report (Chinese)](pixel8a/真机速度对比.md) preserves both rounds, load/first-call times, PSS, temperature, parity and raw timings.
...
[Five-model host report](host.md) · [amd64 control](host-amd64.md). Speed ratios do not transfer across devices.
...
The phone also compares the first image and text for ONNX/ORT in each round; see [final verification](verification.json). Other inputs are timed and checked for finite 512-dimensional outputs, but their phone vectors are not individually archived. These checks are not labeled accuracy. The [historical accuracy report (Chinese)](../识别精度对比.md) retains its original runtime and dataset scope; accuracy was not rerun in this format-comparison experiment.
...
See the [model documentation](../../README.md) for Python environments, exports and host/device commands. This experiment added export/benchmark artifacts and did not change the Android app's production model selection.

The Windows uploader stages files with `adb push` and `run-as cp`, waits for completion and verifies sizes and SHA256. Earlier `exec-in dd` attempts returned before writes completed, and shell stdin truncated binary data. Staging files are removed after copying.

- [ARM export](export-arm.json) and [amd64 export](export-amd64.json): conversion settings, dependencies, hashes and per-input errors.
- [Script checks](script-validation.json): 32 offline checks, including 29 malformed-input rejections and the previous summarizer regression.
- [Export target guards](export-target-guards.json): default ARM and separate amd64 destinations pass; amd64 without a separate destination or targeting ARM assets is rejected.
- [Final verification](verification.json): target hashes, identical host/phone artifacts, sample counts, numerical parity and historical script hashes.
- [APK reuse and installed-file verification](pixel8a/build-verification.json): this experiment reused the APK from the dependency upgrade and made no Android code changes.

Archived JSON contains the measurement evidence and hashes. Model binaries and generated fixture/feature files are excluded from Git; a fresh checkout must regenerate them before rerunning inference. Historical script hashes identify the implementation used for that experiment and need not match later cleanup revisions.
