...
class MobileCLIPVersionsBenchmarkTest {
    ...
    fun exportAccuracyFeatures() {
        ...
        val report = JSONObject()
            ...
            .put("session_configuration", JSONObject()
        ...
        try {
            ...
            for ((tower, modelFile) in listOf("image" to imageModel, "text" to textModel)) {
                ...
                BenchmarkSession(modelFile, "onnx", image, 256, 4, "default", unusedProbe).use { session ->
                    report.put("${tower}_session", JSONObject().put("load_ms", elapsedMs(loadStart))
    ...
    private fun verifyAccuracyFile(
        root: File, row: JSONObject, fileKey: String, hashKey: String, expectedBytes: Long? = null
    ): File {
    ...
    private fun exportAccuracyDataset(
        session: BenchmarkSession, inputFile: File, outputRoot: File, modelId: String,
        dataset: JSONObject, tower: String, count: Int
    ) {
    ...
    private fun benchmarkTower(
        context: Context, file: File, backend: String, image: Boolean, size: Int, inputs: List<ByteBuffer>,
        sampleCount: Int, warmupCount: Int, cpuThreads: Int, xnnpack: String, probeLibrary: File,
        report: JSONObject, health: JSONArray, name: String
    ) {
    ...
    private fun sustain(
        context: Context, session: BenchmarkSession, inputs: List<ByteBuffer>, seconds: Int,
        result: JSONObject, health: JSONArray
    ) {
    ...
    private class BenchmarkSession(
        file: File, backend: String, image: Boolean, size: Int, cpuThreads: Int, xnnpack: String,
        probeLibrary: File
    ) : Closeable {
        ...
        fun prepareInputs(inputs: List<ByteBuffer>): List<ByteBuffer> {
            val prepared = if (type == OnnxJavaType.INT64) inputs.map { source ->
        ...
        private fun matches(actual: LongArray, expected: LongArray): Boolean =
            actual.size == expected.size && actual.indices.all { actual[it] <= 0L || actual[it] == expected[it] }
    }

    /** Diagnostic only: a host-built JNI probe reuses the app's existing LiteRT native library. */
    private class NativeXnnpackDelegate(libraryFile: File, cpuThreads: Int) : Delegate, Closeable {
