...
def report(data, manifest, output, results_path):
    ...
    lines = ["# MobileCLIP v1 / v2 识别精度实测", "", f"报告生成日期：{date.today().isoformat()}。CPU {data['threads']} 线程，模型不训练、不微调。", "",
    ...
    lines += ["", "## 方法与边界", "",
              ...
              "", "## 可复核文件", "",
              f"- [本次原始指标、逐图预测、模型哈希和依赖版本](<{results_path.name}>)",
              f"- [本次样本 ID、标签、哈希和全部查询](<{results_path.stem}-manifest.json>)",
              "- [自动选择的互有胜负样例](accuracy-disagreements.jpg)",
              "", "这些是本地生成的报告与输入，请保存在 Git 忽略的 build/ 目录。v1 的源权重与导出校验记录位于各模型输出目录的 metadata.json。",
...
def main():
    ...
    parser.add_argument("--results", type=Path, default=Path("build/mobileclip-accuracy/accuracy-results.json"))
    ...
    report(data, manifest, args.results.with_name("识别精度对比.md"), args.results)
