...
def render(verification, accuracy, args, manifest_path, language="en"):
    ...
    lines += ["", t("## Evidence and reproduction", "## 可复核文件"), "",
              link(t("Published measurement summary", "已发布测量摘要"), archive.parent / "measured-summary.json", args.output) + " · " + link(t("Phone accuracy summary", "手机准确率摘要"), archive / "pixel8a-accuracy" / t("README.md", "README_zh.md"), args.output), "",
              ...
              t("This generated report links to local measurements, predictions, manifests and validation files. Keep those artifacts under build/, which is excluded from Git. The repository retains curated aggregate summaries; independently reproducing predictions or rankings requires rerunning inference.", "本次生成的报告链接本地测量、预测、清单与验证文件。请将这些产物保存在 Git 忽略的 build/ 下。仓库仅保留整理后的聚合摘要；独立复核逐图预测或排序需要重新运行推理。")]
    if args.device_dir:
