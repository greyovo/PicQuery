...
def provenance_path(path):
    """Describe inputs without publishing local usernames or checkout locations."""
    resolved = path.resolve()
    return resolved.relative_to(ROOT).as_posix() if resolved.is_relative_to(ROOT) else resolved.name
...
def verification_inputs(args, np, source_hashes):
    ...
    provenance = {"golden_reference": {"path": provenance_path(reference), "sha256": sha256(reference),
    ...
    provenance["image_fixtures"] = [{"path": provenance_path(p), "sha256": sha256(p)} for p in args.image_fixture]
    if args.verification_manifest:
        ...
        provenance["manifest"] = {"path": provenance_path(manifest_path), "sha256": sha256(manifest_path),
