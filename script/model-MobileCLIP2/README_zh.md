...
本指南说明当前 Android 模型契约、ONNX/TFLite/ORT 导出方法，以及独立的精度和计时实验。所有命令从仓库根目录运行。新实验输出写入被忽略的 `build/` 目录。保留的摘要描述历史模型与运行库，不分发原始测量文件。
...
导出器 metadata 中的输入来源使用仓库相对路径；checkout 外的输入以文件名和 SHA256 标识，不记录本机绝对路径。
...
校准使用 400 张训练图：CIFAR-100 每类两张、Imagenette 每类 20 张，种子 20260913。原始文件与解码 RGB 哈希均检查与全部 5,925 张保留评测图不重叠。保留层的选择依据训练输入误差，而非评测标签。最初全算子方案明显损害向量一致性；保留的[量化摘要](results/image-int8/README_zh.md)区分该方案与最终混合候选，不包含其原始诊断文件。
...
Git 保留[聚合测量、模型哈希和协议](results/measured-summary.json)，不包含原始日志、逐图预测、输入清单或特征数组。重算排序与配对统计需要在 `build/` 下重新生成相应输入输出。
...
请对当前 checkout 执行检查，将生成日志保存在 `build/` 下。离线回归验证代码和保留摘要的一致性，不会重新得到历史精度或速度测量。
...
默认精度配置包含九项，其中有离线 FP32 文本参照。四个下载 ZIP 均不包含这些参照文件。`prepare_accuracy_models.py` 会生成 v1 参照；v2 参照还需要通过上方 ONNX 源导出器生成 `build/mobileclip2-reference/mobileclip2_s0_text.onnx`。TFLite 比较项需要 Python `ai-edge-litert==2.1.4`，与 Android 的 LiteRT 依赖分开。

在完整 Python 环境按下列顺序执行。v2 源导出将部署用 ONNX 写到 `build/`，在生成缺失 FP32 文本参照的同时保留已下载的 App/ORT 模型对：
...
ORT 比较矩阵需要 v1 导出、当前 v2 ONNX/ORT 模型对和历史 CLIP 资产。[历史 v1/v2 精度摘要](results/README_zh.md#历史-onnxtflite-准确率)使用较早运行库；[ORT 对比](results/ort-comparison/README_zh.md)记录自身 ORT 1.29.0 主机/手机协议。重跑必须记录实际版本和哈希，不要把历史精度与新计时合成同一配置的结论。
...
[测量索引](results/README_zh.md)区分历史协议与保留的聚合结果。较早 App/优化测量使用 Android ONNX Runtime 1.23.2 / LiteRT 1.4.1，包含最初 Java lazy delegate 的线程问题及后续显式原生 delegate 修正。这些观察不能代表当前运行库性能。仅保留精简测量 JSON 与中英双语摘要，原始诊断报告需在本地重新生成。
