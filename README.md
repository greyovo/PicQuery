...
[Compact measured data](script/model-MobileCLIP2/results/measured-summary.json) retains aggregate results, model hashes and protocols. Raw benchmark/build logs and per-image outputs are not included; use the model guide to generate new local results under `build/`.
